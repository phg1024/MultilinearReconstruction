...
int main(int argc, char *argv[]) {
  ...
  desc.add_options()
    ...
    ("init_weights_file", po::value<string>()->required(), "Initial multilinear weights file.")
  ...
  string init_weights_file, init_recon_path;
  int iteration;
  ...
  try {
    ...
    if(vm.count("maxiters")) opt_params.max_iters = vm["maxiters"].as<int>();
    ...
    init_weights_file = vm["init_weights_file"].as<string>();
  ...
  auto valid_faces_indices_quad = LoadIndices(home_directory + "/Data/Multilinear/face_region_indices.txt");
  ...
  // Load initial multilinear weights
  vector<double> init_wid, init_wexp;
  {
    ifstream fin(init_weights_file);
    int dim_wid;
    fin >> dim_wid;
    init_wid.resize(dim_wid);
    for(int i=0;i<dim_wid;++i) fin >> init_wid[i];

    int dim_wexp;
    fin >> dim_wexp;
    init_wexp.resize(dim_wexp);
    for(int i=0;i<dim_wexp;++i) fin >> init_wexp[i];
  }
  ...
  for(auto& p : image_points_filenames) {
    ...
    if(pure_syn_mode) {
      ...
      recon.SetOptimizationMode(
        ...
        //| SingleImageReconstructor<Constraint2D>::Identity
    ...
    if(pure_syn_mode) {
      ...
      // Update the idenity weights
      for(int i=0;i<50;++i) recon_results.params_model.Wid(i) = init_wid[i];
